...
const scenarios = testConfig.getScenarios().filter(s => s.scenario.includes('Bundle product'));

test.describe('Bundle Product Order Tests', () => {
			...
			for (const testCaseName of testCases) {
				test(`Execute: ${testCaseName}`, async ({ baseTest }, testInfo) => {
					...
					try {
						...
						currentStep = 'Add Bundle Product';
                        
						await test.step(currentStep, async () => {
                            await addOrderPage.clickNextButton();
							// Example: customize and add IBM Assessment Voucher Bundle
							const bundleName = typeof testCase?.['Bundle_Name'] === 'string' ? testCase['Bundle_Name'] : '';
							const useStorePricing = testCase?.['Use_Store_Pricing'] === false ? false : true;
							const bundleQuantity = typeof testCase?.['Bundle_Quantity'] === 'string' ? Number(testCase['Bundle_Quantity']) : 1;
							const product1 = typeof testCase?.['Product_1'] === 'string' ? testCase['Product_1'] : '';
                            const product2 = typeof testCase?.['Product_2'] === 'string' ? testCase['Product_2'] : '';
                            const product3 = typeof testCase?.['Product_3'] === 'string' ? testCase['Product_3'] : '';
                            console.log('Products to add in bundle:', product1, product2, product3);
							await addOrderPage.customizeAndAddIBMBundle({
